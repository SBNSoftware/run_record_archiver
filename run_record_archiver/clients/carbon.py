    ...
    def __init__(
        ...
        if self.enabled and not (self.host and self.port and self.metric_prefix):
            self._logger.warning(
                "Carbon client enabled but missing required configuration."
            )
            self.enabled = False

    def post_metric(
        self, metric_path: str, value: float, timestamp: Optional[float] = None
    ) -> None:
        if not self.enabled:
            return
        ...
        try:
            ...
            self._logger.warning(
                "Could not post metric '%s' to Carbon at %s:%d. Reason: %s",
                full_metric_path,
                self.host,
                self.port,
                e,
